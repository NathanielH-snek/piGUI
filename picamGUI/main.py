...
def startVideo():
	...
	if not encoderON:
		encoderValue = indexQuality()
		match encoderValue:
			case 0:
				encoderQual = Quality.VERY_LOW
			case 1:
				encoderQual = Quality.LOW
			case 2:
				encoderQual = Quality.MEDIUM
			case 3:
				encoderQual = Quality.HIGH
			case 4:
				encoderQual = Quality.VERY_HIGH
		picam2.start_recording(encoder,output,encoderQual)
		encoderON = True
...
class myApp(QMainWindow):
	...
	def initUI(self):
		...
		def getSavepath():
			...
			dialog.exec()
			outputPath = dialog.selectedFiles()
			outputPath = re.sub('\ |\[|\]','',str(outputPath)).strip('"\'')
			savePathDisplay.setText(f"Recording Path: {outputPath}")
		...
		global indexQuality
		def indexQuality():
			return self.qualityDropdown.currentIndex()
		...
		savePathButton = QPushButton("Change Filepath")
		savePathButton.clicked.connect(getSavepath)
		pageLayout.addWidget(savePathButton,0,1)
		
		savePathDisplay = QLabel()
		savePathDisplay.setText(f"Recording Path: {outputPath}")
		pageLayout.addWidget(savePathDisplay,0,2,1,-1)
		...
		self.qualityDropdown.addItems(['VERY_LOW','LOW','MEDIUM','HIGH','VERY_HIGH'])
		pageLayout.addWidget(self.qualityDropdown,25,1,1,1)
